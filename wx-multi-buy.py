...
myBuyHeaders = [[None, {}]] # 多账号查询余票


IndexMonitor = 0

time_wait = 0 # (55 / (12 * len(myHeaders)))


while True:
    myAnswer = None
    while myAnswer != "Y" and myAnswer != "N":
        myAnswer = input("是否继续添加主号？(Y：是，N：否)：")
    ...
    while True:
        try:
            ...
            print(e)
            pass
    myBuyHeaders.append([hzmbus, headers])
    writeLog("[主号 OAuth 登录] 已登录主号 " + str(IndexMonitor) + "。")
    IndexMonitor += 1
...
while True:
    ...
    while myAnswer != "Y" and myAnswer != "N":
        ...
        if myAnswer == "N" and IndexMonitor == 0:
            print("您选择了不添加副号，主号将成为您的副号。")
            myHeaders = myBuyHeaders
    if myAnswer == "N":
        break
    hzmbus = None
    headers = None
    ...
    while True:
        ...
        while True:
            try:
                if bought:
                    sys.exit(0)
                homepage = hzmbus.post("https://wx.hzmbus.com/webapi/ticket/buy.ticket", headers=headers, json=myhashs[i].set_token_web({
                "ticketData": DATE,
                "lineCode": ROUTE,
...
DF = "%Y-%m-%d"
oldtime_wait = 0
FINISHEDCAPTCHAS = [False for i in range(len(myBuyHeaders))]
ALLFINISHEDCS = [True for i in range(len(myBuyHeaders))]
my_caps = [{"sessionId": "", "sig": "", "token": ""} for i in range(len(myBuyHeaders))];myhashs = [hzmbus_hash.HZMHash() for i in range(len(myBuyHeaders))]
writeLog("[提示] 等待中...")
while True:
        ...
        if CAPTCHA == 2:
            ...
            def getTime(i, TIMESHUA, FINISHEDCAPTCHAS, my_caps, myhashs, hzmbus, headers):
                while True:
                    ...
                    break
                FINISHEDCAPTCHAS[i] = True
            for i in range(len(myBuyHeaders)):
    ...
    if (weekday != 1) or (hour >= eightPM):
        ...
        while True:
            ...
            CAPTCHAS = [CAPTCHA for i in range(len(myBuyHeaders))]
            CWRONGS = [CWRONG for i in range(len(myBuyHeaders))]
            ali_being_used = False
            bought = False
            for i in range(len(myBuyHeaders)):
